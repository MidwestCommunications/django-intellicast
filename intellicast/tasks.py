...
def update_map_images():
    ...
    #Do it all over with new image for nashville area
    r = requests.get('http://services.intellicast.com/200904-01/158765827/Image/Radar/Radar2009.13L/Loop/SectorName/r07')
    ...
    while True:
        try:
            ...
            for size, key_dict in settings.INTELLICAST_CROP_DICT_SOUTH.items():
                for key, crop in key_dict.items():
                    frame_list = frames_dict.setdefault(key, [])
                    frame_list.append(original_file.copy().crop(crop).resize(size))
